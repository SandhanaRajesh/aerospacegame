...
@onready var life: TextureRect = $LifeContainer/Life1
...
func _ready() -> void:
	await Timer(3.5) # using the function created
	
	if Global.minigames_done < 5: # if you havent completed 3 minigames yet 
		Global.minigames_done = Global.minigames_done +1
		get_tree().change_scene_to_file("res://minigame_" + str(Global.minigames_done) + "_info_screen.tscn") # changes your scene by arranging this frankenstein path. 
